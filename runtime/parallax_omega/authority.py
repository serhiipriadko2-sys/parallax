...
class ActionGovernor:
    ...
    def decide(
        ...
        *,
        ...
        if request.risk is RiskLevel.R4 and graph.verified_source_class_count(
            request.evidence_claim_ids,
            at=at,
        ) < 2:
            missing.append("independent_source_classes:2")

        if request.risk in {RiskLevel.R2, RiskLevel.R3, RiskLevel.R4}:
        ...
        if request.risk in {RiskLevel.R3, RiskLevel.R4}:
        ...
        if request.irreversible or request.risk is RiskLevel.R4:
            if not context.dual_control:
                return ActionDecision(
